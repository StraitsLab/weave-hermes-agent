...
class CDPSupervisor:
    ...
    def focus_page(self, origin: str, *, accept: Optional[str] = None, timeout: float = 10.0) -> Dict[str, Any]:
        ...
        Ported from upstream ``CDPSupervisor.focus_page`` @49b4286a22 (1bd4e33d36); the fork has no
        ``_enable_page_domains`` / ``_schedule`` helpers, so the attach sequence mirrors
        ``_attach_initial_page`` and scheduling mirrors ``evaluate_runtime``.
        """
        loop = self._loop
        ...
        async def _attach(target_id: str) -> str:
            ...
            await self._cdp("Page.enable", session_id=sid, timeout=timeout)
            await self._cdp("Runtime.enable", session_id=sid, timeout=timeout)
            await self._cdp("Target.setAutoAttach", {"autoAttach": True, "waitForDebuggerOnStart": False, "flatten": True},
        ...
        try:
            fut = safe_schedule_threadsafe(_focus(), loop)
            if fut is None:
                return {"ok": False, "error": "Browser supervisor loop unavailable"}
