...
def dock_argv(exe: str, user_data_dir: str) -> list[str]:
    ...
    # Fork: a preset AGENT_BROWSER_ARGS is the policy agent-browser starts this binary with; the dock uses it too.
    preset = [a.strip() for a in os.environ.get("AGENT_BROWSER_ARGS", "").split(",") if a.strip()]
    return [exe, f"--user-data-dir={user_data_dir}", "--remote-debugging-port=0", "--no-first-run",
            ...
            *(preset or (CHROMIUM_SANDBOX_BYPASS_ARGS if _needs_chromium_sandbox_bypass() else ()))]
