...
def _eval_js_secret(task_id: str, expression: str) -> Dict[str, Any]:
    ...
    # Re-admit at the WRITE. The handler-level fence (_fenced_page_op) admitted before a possibly human-length
    # prompt (enter_code waits for the user's code); a takeover during that wait must refuse here, before the
    # credential lands in a page the human is now typing into. The outer epoch check only discards the result.
