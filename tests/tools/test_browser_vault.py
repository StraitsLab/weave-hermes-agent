...
class TestBrowserVaultTools:
    ...
    def test_secret_eval_fails_closed_without_supervisor(self, store):
        ...
        with patch("agent.vault_store.get_vault_store", return_value=store), \
             ...
             patch("tools.browser_tool._run_browser_command", return_value={"success": False}) as run_cmd:
...
class TestManagerAutoDetection:
    def test_only_the_local_vault_is_a_login_source_in_this_fork(self):
        """Fork port: the 1Password / Bitwarden backends are not ported, so an installed manager CLI never
        becomes a login source and config cannot turn one on (retargeted from upstream's auto-detection test)."""
        from agent.vault_backends import base

        assert base.external_backend_classes() == ()
        with patch.object(base, "is_installed", return_value=True), \
             patch.object(base, "_cfg", return_value={"bitwarden": {"enabled": True}, "onepassword": {"enabled": True}}):
            assert [b.name for b in base.enabled_backends()] == ["local"]


def test_every_vault_tool_is_in_the_browser_toolset():
    """A tool registered but in no toolset is invisible to the model (live: browser_vault_save_login was
    registered, tested, and never offered). Fork: the vault tools join `browser` through their registry
    registration, not the static list, so the platform subset inference over static composites is unchanged."""
    import toolsets
    ...
    assert registered, "vault tools did not register"
    browser = set(toolsets.resolve_toolset("browser"))
    assert registered <= browser, registered - browser
