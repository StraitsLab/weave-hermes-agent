...
def test_callable_credential_is_redacted_refreshable_and_expiring():
    holder = server.SessionCredential("first-bearer", datetime.now(timezone.utc) + timedelta(minutes=1))
    ...
    expired = server.SessionCredential("expired-bearer", datetime.now(timezone.utc) - timedelta(seconds=1))
    with pytest.raises(RuntimeError, match="credential unavailable"):
        expired()
...
def test_holder_present_before_agent_activation_replaces_static_key_once():
    holder = server.SessionCredential("deferred-bearer", datetime.now(timezone.utc) + timedelta(minutes=1))
...
def test_bound_sessions_reject_untrusted_native_and_stored_key_dispatches_then_revoke_on_close():
    holder = server.SessionCredential("close-bearer", datetime.now(timezone.utc) + timedelta(minutes=1))
    agent = _Agent()
